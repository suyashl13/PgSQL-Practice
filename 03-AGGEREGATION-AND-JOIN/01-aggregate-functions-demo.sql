...
-- Getting count of photos for each user.ABORT
SELECT
  user_id,
  count(*)
FROM
...
-- Getting count of photos for each user with alias and join with users
SELECT
  users.username,
  count(*)
FROM
...
select c.photo_id , count(*) as photo_count
from "comments" c
where photo_id < 4
group by c.photo_id
having count(*) < 20;

-- using having clause and sum
select phones.manufacturer, sum(price * units_sold)
from phones
group by phones.manufacturer
